...
class Task:
    ...
    def __repr__(self):
        return "Task: {}\n(Priority: {},\tDue: {:02d}/{:02d}/{},\tComplete: {})".format(self.__description,
                                                                                        self.get_priority_as_words(),
                                                                                        self.__due_date.day,
                                                                                        self.__due_date.month,
                                                                                        self.__due_date.year,
                                                                                        self.__complete)
...
class ToDoApp:
    ...
    def __init__(self):
        ...
        self.__tasks.append(Task("Do prep", "3", "27/11/2018"))
    ...
    def save_tasks(self):
        filename = input("Enter a filename for the tasks file\n> ")

        with open(filename, 'wb') as task_file:
            pickle.dump(self.__tasks, task_file)

        print("Tasks saved!")
        input("\n\nPress Enter to return to the main menu...")

    def load_tasks(self):
        filename = input("Enter the filename of the tasks file that you wish to open\n> ")

        confirm = input(
            "This will replace all existing tasks with those in the file. Are you sure you want to do this (y/n)? ")

        if confirm.upper() == "Y":
            try:
                with open(filename, 'rb') as task_file:
                    self.__tasks = pickle.load(task_file)
                print("Tasks loaded!")
            except FileNotFoundError:
                print("Could not find a tasks file named {}".format(filename))
        else:
